...
export default function ExchangeDecisionPopUp({ exchange, order, action }) {
  ...
  const onDesicion = (decision) => {
    ...
    formData.append("ExchangeId", exchange.data.data.id);
    formData.append("ResponseExchange", response);
    formData.append("Status", decision);

    editExchangeRequest.mutate(formData, {
      onSuccess: (response) => {
  ...
  return (
    <StyledPopUp action={() => {}}>
      ...
      <Container>
        ...
        <Buttons>
          <button onClick={() => onDesicion("Success")}>Accept and create new Order</button>
